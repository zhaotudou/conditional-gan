...
dataloader = torch.utils.data.DataLoader(
    datasets.MNIST('../../data', train=True, download=True,
                   transform=transforms.Compose([
                       ...
                       transforms.ToTensor(),
                      # output with shape [1, 28, 28] doesn't match the broadcast shape [3, 28, 28]报错修改下面这一行
...
for epoch in range(args.n_epochs):
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        # 在g阶段，输入的是随机采样的噪声z和随机生成的类别
        # 在d阶段，输入的是真实图片和真实图片对应的类别生成的条件变量
        #              以及假的生成图片和假图片生成时对应随机的生成的类别
        # 训练阶段要做的是，将这种带有类别信息的图片判断是真还是假，那么生成的带有类别的信息也有真和假，如果为真，则说明生成的图片和指定的类别是符合的，最后生成的图片也是近似真实图片的
        ...
        if batches_done % args.sample_interval == 0:
            ...
            y_fixed = Variable(y_fixed.scatter_(1,y_.view(N_Class**2,1),1).cuda())  # N_Class**2 =100
